...
The string <code>'a'</code> passed to assert changes is evaluated in the block context, both before and after the block is run. 
...
When given just one value, the precondition is that the value does NOT equal the value;
the post-condition is that it now equals the specified value:
